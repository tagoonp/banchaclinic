...
if($searchkey != ''){
    $strSQL = "SELECT * FROM bcn_patient a LEFT JOIN bnc_patient_log b ON a.patient_id = b.pl_patient_id
           WHERE 
           (a.patient_fname LIKE '$searchkey%' 
           OR a.patient_lname LIKE '$searchkey%' 
           OR a.patient_hn LIKE '$searchkey%' 
           OR a.patient_pid LIKE '$searchkey%')
            ...
            <div class="content-header row">
                <div class="content-header-left col-12 mb-2 mt-1">
                    <div class="breadcrumbs-top">
                        <h5 class="content-header-title float-left pr-1 mb-0 text-dark" style="font-size: 28px;">ระบบคิดเงิน</h5>
                        <div class="breadcrumb-wrapper d-none d-sm-block" style="padding-top: 10px;">
                            <ol class="breadcrumb p-0 mb-0 pl-1">
                                <li class="breadcrumb-item"><a href="./"><i class="bx bx-home-alt"></i></a>
                                </li>
                                <li class="breadcrumb-item"><a href="pp-cashing.php">ระบบคิดเงิน</a></li>
                                <li class="breadcrumb-item active">ค้นหาผู้ป่วย</li>
                            </ol>
                        </div>
                    </div>
                </div>
            ...
            <div class="content-body row">
                            ...
                            <div class="mb-2">
                                <div class="row">
                                        ...
                                        <fieldset>
                                            <form id="searchForm" onsubmit="return false;">
                                                <div class="input-group">
                                                    <input type="text" class="form-control" id="txtSearchkey" name="txtSearchkey" placeholder="ค้นหาด้วยชื่อ นามสกุล หมายเลขผู้ป่วย หรือ เลขบัตรประจำตัวประชาชน" aria-describedby="button-addon2" autofocus value="<?php if($searchkey != ''){ echo $searchkey; }?>">
                                                    <div class="input-group-append" id="button-addon2">
                                                        <button class="btn btn-primary" type="submit"><i class="bx bx-search"></i> ค้นหา</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </fieldset>
                                    ...
                                    <div class="col-4 text-right">
                                        <button class="btn btn-danger pl-1" data-toggle="modal" data-target="#modalNewPatient"><i class="bx bxs-user-plus"></i> เพิ่มผู้ป่วยใหม่</button>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="card">
                        <div class="card-body p-0">
                            <div class="p-2"><h3>รายชื่อผู้ป่วยที่ยังไม่คิดเงิน/ค้างจ่าย</h3></div>
                            <div class="table-responsive" style="height: 500px; overflow: scroll;">
                                <table class="table table-striped th" style="margin-top: 0px;">
                                    <thead>
                                        <tr>
                                            <th>ชื่อ - นามสกุล</th>
                                            <th></th>
                                    ...
                                    <tbody>
                                        <?php 
                                        $strSQL = "SELECT * FROM bcn_patient a INNER JOIN bnc_service b ON a.patient_id = b.service_patient_id
                                        WHERE
                                        a.patient_delete = '0'
                                        AND a.patient_record_status = '1'
                                        -- AND b.service_date = '$date' 
                                        AND b.service_status NOT IN ('discharge')
                                        AND b.service_delete = '0'
                                    ";
                                    ...
                                    $resWaiting = $db->fetch($strSQL, true, true);
                                        if($resWaiting['status']){
                                            foreach($resWaiting['data'] as $row2){
                                                ...
                                                <tr>
                                                    <td style="vertical-align: top;">
                                                        <span class="badge badge-primary"><?php echo $row2['patient_hn']; ?></span><br>
                                                        <a href="app-patient-info.php?pid=<?php echo $row2['patient_id'];?>" class="text-dark" style="font-size: 1.3em;"><?php echo $row2['patient_fname']." ".$row2['patient_lname']; ?></a>
                                                        <div>วันที่ : <?php echo $row2['service_date']; ?></div>
                                                    </td>
                                                    <td style="vertical-align: top;" class="text-right">
                                                            <button class="btn btn-icon rounded-circle btn-outline-success" style="height: 40px; width: 40px; padding-top: 1px; margin-bottom: 3px;" onclick="window.location='app-viewrecord.php?patient_id=<?php echo $row2['patient_id']; ?>'"><i class="bx bx-search" style="font-size: 1.3em;"></i></button>
                                                            <button class="btn btn-icon rounded-circle btn-outline-danger" style="height: 40px; width: 40px; padding-top: 1px; margin-bottom: 3px;" onclick="deleteService('<?php echo $row2['service_id']; ?>')"><i class="bx bx-trash" style="font-size: 1.3em;"></i></button>
                                                    </td>
                                                </tr>
                                                <?php
                                            }
                                            ...
                                            ?>
                                            <tr>
                                                <td colspan="2" class="text-center">ไม่มีรายชื่อผู้ป่วยตกค้าง</td>
                                            </tr>
                                            <?php
                                        }
                                        ?>
